...
export function SankeyWidget({ onMaximize, onRemove, onDuplicate, minimal = false, isDraggable = false }: { onMaximize?: () => void; onRemove?: () => void; onDuplicate?: () => void; minimal?: boolean; isDraggable?: boolean }) {
  ...
  const getNodeDetailData = (nodeId: string, totalValue: number) => {
    ...
    if (nodeId === 'abandoned_queue_b') {
      const openBase = 400;
      const closedBase = 37;
      const digitalBase = 342;
      const totalBase = openBase + closedBase + digitalBase; // 779

      return [
        { label: 'Open hours', value: Math.round(totalValue * (openBase / totalBase)), icon: '📞' },
        { label: 'Closed hours', value: Math.round(totalValue * (closedBase / totalBase)), icon: '📞' },
        { label: 'Digital abandoned', value: Math.round(totalValue * (digitalBase / totalBase)), icon: '💬' },
      ];
    }
